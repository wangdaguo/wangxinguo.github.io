...
`digest`：MD5 哈希值的字节数组。MD5 哈希值的长度是 128 位（16 字节）。

`idx`：用于选择要提取的 32 位段的索引。MD5 哈希值被分成 4 个 32 位的段（每段 4 个字节）。

 逻辑步骤：

1, **从字节数组中提取 4 个字节**：根据 `idx`，从 `digest` 数组中提取连续的 4 个字节。这 4 个字节表示一个 32 位的段。

2, **每个字节与 255 进行按位与操作**：确保每个字节被转换为无符号的整数。这是因为 Java 中的 `byte` 类型是有符号的，而我们需要无符号值。

3, **将 4 个字节组合成一个 32 位的无符号整数**：

- 第一个字节（最低位）直接放在最低 8 位。
- 第二个字节左移 8 位后放在接下来的 8 位。
- 第三个字节左移 16 位后放在再接下来的 8 位。
- 第四个字节左移 24 位后放在最高的 8 位。

4, **与 4294967295L 进行按位与操作**：确保结果是一个 32 位的无符号整数。这是因为 `long` 类型在 Java 中是 64 位的，而我们需要的结果是 32 位无符号整数。
...
- `md5` 方法计算并返回 `rpcServiceKey` 的 MD5 哈希值，长度为 16 字节（128 位）。
- 选择第 0 个部分作为哈希值；
...
`virtualInvokers` 是一个 `TreeMap<Long, String>`，存储虚拟节点的哈希值及其对应的实际服务地址。

- `tailMap(hashCode, true)` 方法返回一个视图，该视图包含所有大于或等于 `hashCode` 的键值对。
- `firstEntry()` 方法返回这个视图中的第一个条目，即最接近且大于或等于 `hashCode` 的虚拟节点。

2, 处理边界情况：

- 如果没有找到大于或等于 `hashCode` 的虚拟节点（即 tailMap 为空），这可能发生在 hashCode 超过了哈希环的最大值的情况。

- 在这种情况下，`firstEntry()` 方法返回 `virtualInvokers` 中的第一个条目，即哈希环的第一个节点。

- 这一步确保了哈希环的循环特性，使得哈希环可以首尾相连。
